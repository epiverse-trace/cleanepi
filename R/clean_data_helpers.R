...
#'    The considered data types are: `numeric`, `Date`, `character`
#'
...
get_column_composition <- function(x) {
  ...
  are_character <- round((1 - (are_na + are_numeric + are_date)), 6)

  # return the output
  c(are_na, are_numeric, are_date, are_character)
}

#' Scan a data frame object to determine the percentage of `missing`, `numeric`,
#'    `Date` and `character` values in every column.
#'
#' @param data the input data frame
#'
#' @return a data frame with the same columns as the input data and 4 rows.
...
scan_data <- function(data) {
  ...
  data_type_percent = data.frame(c("missing", "numeric", "date", "character"),
                                 stringsAsFactors = FALSE)
  scan_result <- cbind(data_type_percent, scan_result)
  names(scan_result)[1] <- "data_type_percent"
  scan_result
}
