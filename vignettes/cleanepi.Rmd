...
Note that the function to visualize this report object will be built in the next release of the package.
...
The function returns a `list` of `r length(res)` elements named as:
...
library("magrittr")
head(dups %>% dplyr::select(c(row_id, group_id, as.character(linelist::tags(data))))) 
```
...
The output from the `find_duplicates()` function can also be provided to the `remove_duplicates()` to specify which duplicated rows to be removed.  
