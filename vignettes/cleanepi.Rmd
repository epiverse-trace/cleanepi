...
standardise_col_names <- list(keep = NULL, rename = NULL)
...
params <- list(
  standardize_column_names = standardise_col_names,
...
The `remove_constant()` function can be used to remove such “noise”. The function takes the following argument:
...
dat <- remove_constant(data   = data,
                       cutoff = 1)
```

The `remove_constant()` function returns a dataset where all constant columns, empty rows and columns are removed.
...
The default date format in R is `Ymd` (the ISO format). However, it is very common to encounter date values that are written differently from this. Also, there are cases where a column in a data frame contains both values of type `Date`, `character` or others.   
...
5. **error_tolerance**: The maximum percentage of `NA` values (non date values) that can be allowed in a converted column. Default is 40% i.e. `0.5`.
...
The `check_subject_ids()` function is designed to identify rows from the input dataset where the ids don't comply with the expected subject ids format. It requires the following parameters:
...
By providing these parameters, the function becomes a versatile tool for data cleaning, ensuring that the user is alerted on the presence of unexpected subject ids. When using the function, make sure to tailor the parameters according to the specific requirements of your dataset and the expected characteristics of the subject IDs.
...
1. **data**: The input data frame or linelist from which duplicated rows will be removed.
...
The `span()` function computes the time span between a single Date value or a vector Date values or another column of the target data and a chosen in the target data. The resulting time span can be expressed in “years”, “months”, “weeks”, or “days”, depending on the user-specified unit. The functions requires the following arguments:

1. **data**: The input dataset (required).
2. **target_column**: A string with the name of the target column (require). The values in this column are expected to be in the form of `Ymd` i.e. `2024-01-31`. The time span will calculated between these values and the reference value(s).
3. **end_date**: it can be either a character that is the name of another column of type Date from the input data or a vector of Date values or a single Date value (required). This should also be in the ISO format ("2024-01-31") and its default value is today's date `Sys.Date()`.
4. **span_unit**: This parameter determines the unit in which the time span is expressed (required). It  can be specified as “years”, “months”, “weeks”, or “days”. By default, the time span is calculated in “years” if this parameter is not provided.
5. **span_column_name**: A string for the name of the column added to the input data. The default is `span`.
6. **span_remainder_unit**: A parameter used to determine the unit in which remainder of the time span calculation will be returned. The possible units are: “days” or “weeks” or “months”. By default, the function returns decimal age i.e. `span_remainder_unit = NULL`.
...
```{r eval=TRUE}
# IMPORT DATA, REPLACE MISSING VALUES WITH 'NA' & STANDARDISE DATES
data <- readRDS(system.file("extdata", "test_df.RDS", package = "cleanepi")) |>
...
2. **Remaining number of days**:  Indicates the remaining number of days after calculating the  time span, representing the fractional part of the time span calculation. This column is included if needed, and provides additional granularity in the time span representation.
