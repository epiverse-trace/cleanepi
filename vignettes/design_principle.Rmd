...
-   **Input:** 
    ...
    -   Tolerance threshold that defines the % of missing (missing and out of range values converted to `NA`) values to be allowed in a converted column. When % missing values exceeds or is equal to it, the original values are returned (default value is 40%)
    -   format (default value is NULL)
    -   timeframe (default value is null)
-   **Output:** 
    -   Returns the input object with standardized date values formatted as *%Y-%m-%d*.
-   **Report:**
