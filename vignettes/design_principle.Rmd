...
* checking the sequence of date-events and
...
**8. Checking order of sequence of date-events **
The functions in this module will allow to check whether the sequence of date events is as expected. For example, it will flag out rows where the date of admission to hospital is prior to the individual date of birth.
    
  * main function: **check_date_sequence()**  
  * Input: a `data.frame` or `linelist` object; a vector of date column names to be
    considered; and a logical to specify whether to remove the incorrect rows from the input      data or not.
  * Output: the input object where the incorrect rows are removed if specified.
  * Report: a table with the incorrect rows from the specified columns. 
  *	Mode: explicit.
