...
- **Input:** Accepts a `data.frame` or `linelist` object, along with:
  ...
  * A vector of strings representing the current missing values (default value is cleanepi::common_na_strings)
...
This module is dedicated to detect date values from character columns and convert them into `Date` value in `ISO8601` format, and ensuring that all dates fall within the given timeframe.
