...
In {cleanepi}, we consider each of them as a **module**, that will have *one main function and helpers* to achieve a specific task. The main function of each module will be exported. This will provide the users with the ability to perform the cleaning task individually when needed.

In the heart of {cleanepi}, there will be `clean_data()`, a wrapper around all the modules. We expect it to be main function to be called by users to clean their data.
...
![cleanepi modules and main functions](../man/figures/cleanepi_design_diagram.drawio.png)
...
The {cleanepi} R package, is developed to provide functional programming-style data cleansing tasks. We considered some cleaning actions as **implicit** (they will be performed whether the user specified it or not), others as **explicit** (performed only on the user's request). In this section, we provide a detailed description of the way that every module is built.
...
This module contains functions to harmonize the style and format of column names in the target dataset, and optionally allows users to specify subset of focal columns i.e. a list of column names to keep as-is.
  ...
  * main function: **standardize_columns()**  
  ...
  * Output: the input object where the column names are standardized.
...
This module is to standardize and unify the text representing the missing values in the dataset. 
  ...
  * Input: a `data.frame` or `linelist` object; a `vector` of column names (if
    not provided, the operation will be performed across all columns); a string
