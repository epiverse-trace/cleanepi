...
**{{ packagename }}**  is an R package to clean, curate, and standardize tabular-data, specifically designed for  epidemiological data 
...
**{{ packagename }}** can also standardize column entries into specified formats, calculates age (in years, months, weeks, or days) from a given `Date` column and a reference date.  

**{{ packagename }}** expects input data in  a form of  data frame-like structure (`data.frame`, `tibble`, etc) or a `linelist` object and returns a processed data of the same type. It also returns an object of type `list` that reports the outcomes from every cleaning task.  
